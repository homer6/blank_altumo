...
abstract class BasesfGuardUser extends BaseObject  implements Persistent
{
	 ...
	 * @return     mixed Formatted date/time value as string or wfDate object (if format is NULL), NULL if column is NULL, and 0 if column value is 0000-00-00 00:00:00
	...
	public function getCreatedAt($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->created_at === '0000-00-00 00:00:00') {
			...
			try {
				$dt = new wfDate($this->created_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to wfDate: " . var_export($this->created_at, true), $x);
			}
		}

		if ($format === null) {
			// Because propel.useDateTimeClass is TRUE, we return a wfDate object.
			return $dt;
	...
	public function getLastLogin($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->last_login === '0000-00-00 00:00:00') {
			...
			try {
				$dt = new wfDate($this->last_login);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to wfDate: " . var_export($this->last_login, true), $x);
			}
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($v === null || $v === '') {
			...
			try {
				if (is_numeric($v)) { // if it's a unix timestamp
					$dt = new wfDate('@'.$v, new DateTimeZone('UTC'));
		...
		if ( $this->created_at !== null || $dt !== null ) {
			...
			$currNorm = ($this->created_at !== null && $tmpDt = new wfDate($this->created_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
	...
	public function setLastLogin($v)
	{
		...
		if ( $this->last_login !== null || $dt !== null ) {
			...
			$currNorm = ($this->last_login !== null && $tmpDt = new wfDate($this->last_login)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;
	 ...
	 * @return     PropelCollection|array User[] List of User objects
	 */
	public function getUsersJoinContact($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = UserQuery::create(null, $criteria);
	 ...
	 * @return     PropelCollection|array User[] List of User objects
	 */
	public function getUsersJoinImage($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = UserQuery::create(null, $criteria);
	 ...
	 * @return     PropelCollection|array User[] List of User objects
	 */
	public function getUsersJoinOrganization($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = UserQuery::create(null, $criteria);
