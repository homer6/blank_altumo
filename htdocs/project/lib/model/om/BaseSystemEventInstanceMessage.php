 ...
 */
abstract class BaseSystemEventInstanceMessage extends BaseObject 
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = SystemEventInstanceMessageQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::SYSTEM_EVENT_INSTANCE_ID)) {
            $modifiedColumns[':p' . $index++]  = '`SYSTEM_EVENT_INSTANCE_ID`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::SYSTEM_EVENT_SUBSCRIPTION_ID)) {
            $modifiedColumns[':p' . $index++]  = '`SYSTEM_EVENT_SUBSCRIPTION_ID`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::RECEIVED)) {
            $modifiedColumns[':p' . $index++]  = '`RECEIVED`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::RECEIVED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`RECEIVED_AT`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::STATUS_MESSAGE)) {
            $modifiedColumns[':p' . $index++]  = '`STATUS_MESSAGE`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`CREATED_AT`';
        }
        if ($this->isColumnModified(SystemEventInstanceMessagePeer::UPDATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`UPDATED_AT`';
        }

        $sql = sprintf(
            'INSERT INTO `system_event_instance_message` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`ID`':
                        ...
                        break;
                    case '`SYSTEM_EVENT_INSTANCE_ID`':
						$stmt->bindValue($identifier, $this->system_event_instance_id, PDO::PARAM_INT);
                        break;
                    case '`SYSTEM_EVENT_SUBSCRIPTION_ID`':
						$stmt->bindValue($identifier, $this->system_event_subscription_id, PDO::PARAM_INT);
                        break;
                    case '`RECEIVED`':
						$stmt->bindValue($identifier, (int) $this->received, PDO::PARAM_INT);
                        break;
                    case '`RECEIVED_AT`':
						$stmt->bindValue($identifier, $this->received_at, PDO::PARAM_STR);
                        break;
                    case '`STATUS_MESSAGE`':
