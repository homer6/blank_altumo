 ...
 *
 * @method     StateQuery leftJoinContact($relationAlias = null) Adds a LEFT JOIN clause to the query using the Contact relation
 * @method     StateQuery rightJoinContact($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Contact relation
 * @method     StateQuery innerJoinContact($relationAlias = null) Adds a INNER JOIN clause to the query using the Contact relation
 *
...
abstract class BaseStateQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `NAME`, `ISO_CODE`, `ISO_SHORT_CODE`, `COUNTRY_ID`, `CREATED_AT`, `UPDATED_AT` FROM `state` WHERE `ID` = :p0';
        try {
    ...
    public function filterByContact($contact, $comparison = null)
    {
        if ($contact instanceof Contact) {
            return $this
                ->addUsingAlias(StatePeer::ID, $contact->getStateId(), $comparison);
