 ...
 */
abstract class BaseSystemEvent extends BaseObject 
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = SystemEventQuery::create()
            ...
            if ($this->systemEventInstancesScheduledForDeletion !== null) {
                if (!$this->systemEventInstancesScheduledForDeletion->isEmpty()) {
                    SystemEventInstanceQuery::create()
                        ->filterByPrimaryKeys($this->systemEventInstancesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(SystemEventPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(SystemEventPeer::NAME)) {
            $modifiedColumns[':p' . $index++]  = '`NAME`';
        }
        if ($this->isColumnModified(SystemEventPeer::UNIQUE_KEY)) {
            $modifiedColumns[':p' . $index++]  = '`UNIQUE_KEY`';
        }
        if ($this->isColumnModified(SystemEventPeer::SLUG)) {
            $modifiedColumns[':p' . $index++]  = '`SLUG`';
        }
        if ($this->isColumnModified(SystemEventPeer::ENABLED)) {
            $modifiedColumns[':p' . $index++]  = '`ENABLED`';
        }
        if ($this->isColumnModified(SystemEventPeer::CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`CREATED_AT`';
        }
        if ($this->isColumnModified(SystemEventPeer::UPDATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`UPDATED_AT`';
        }

        $sql = sprintf(
            'INSERT INTO `system_event` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`NAME`':
                        ...
                        break;
                    case '`UNIQUE_KEY`':
						$stmt->bindValue($identifier, $this->unique_key, PDO::PARAM_STR);
                        break;
                    case '`SLUG`':
    ...
    public function setSystemEventSubscriptions(PropelCollection $systemEventSubscriptions, PropelPDO $con = null)
    {
        ...
        foreach ($this->systemEventSubscriptionsScheduledForDeletion as $systemEventSubscriptionRemoved) {
            $systemEventSubscriptionRemoved->setSystemEvent(null);
        }
    ...
    public function removeSystemEventSubscription($systemEventSubscription)
    {
        if ($this->getSystemEventSubscriptions()->contains($systemEventSubscription)) {
            ...
            $systemEventSubscription->setSystemEvent(null);
        }
    }
    ...
    public function setSystemEventInstances(PropelCollection $systemEventInstances, PropelPDO $con = null)
    {
        ...
        foreach ($this->systemEventInstancesScheduledForDeletion as $systemEventInstanceRemoved) {
            $systemEventInstanceRemoved->setSystemEvent(null);
        }
    ...
    public function removeSystemEventInstance($systemEventInstance)
    {
        if ($this->getSystemEventInstances()->contains($systemEventInstance)) {
            ...
            $systemEventInstance->setSystemEvent(null);
        }
    }
