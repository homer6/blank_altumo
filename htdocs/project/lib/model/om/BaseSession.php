...
abstract class BaseSession extends BaseObject 
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(SessionPeer::SESSION_KEY)) {
            $modifiedColumns[':p' . $index++]  = '`SESSION_KEY`';
        }
        if ($this->isColumnModified(SessionPeer::DATA)) {
            $modifiedColumns[':p' . $index++]  = '`DATA`';
        }
        if ($this->isColumnModified(SessionPeer::CLIENT_IP_ADDRESS)) {
            $modifiedColumns[':p' . $index++]  = '`CLIENT_IP_ADDRESS`';
        }
        if ($this->isColumnModified(SessionPeer::SESSION_TYPE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`CLIENT_IP_ADDRESS`':
						$stmt->bindValue($identifier, $this->client_ip_address, PDO::PARAM_STR);
                        break;
                    case '`SESSION_TYPE`':
    ...
    public function setSingleSignOnKeys(PropelCollection $singleSignOnKeys, PropelPDO $con = null)
    {
        ...
        foreach ($this->singleSignOnKeysScheduledForDeletion as $singleSignOnKeyRemoved) {
            $singleSignOnKeyRemoved->setSession(null);
        }
    ...
    protected function doAddSingleSignOnKey($singleSignOnKey)
    {
        ...
        $singleSignOnKey->setSession($this);
    }
    ...
    public function removeSingleSignOnKey($singleSignOnKey)
    {
        if ($this->getSingleSignOnKeys()->contains($singleSignOnKey)) {
            ...
            $singleSignOnKey->setSession(null);
        }
    }
